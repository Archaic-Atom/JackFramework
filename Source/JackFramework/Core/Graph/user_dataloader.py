...
class UserDataloader(object):
    ...
    def user_show_training_info(self, epoch: int, loss: list, acc: list, duration: float,
    ...
    def user_show_intermediate_result(self, epoch: int,
                                      loss: list, acc: list) -> str:
        return self.__jf_datahandler.show_intermediate_result(epoch, loss, acc)

    def user_save_result(self, output_data: list, supplement: list, img_id: int):
