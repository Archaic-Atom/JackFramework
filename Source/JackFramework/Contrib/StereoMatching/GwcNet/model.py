    ...
    def __init__(self, concat_feature=False, concat_feature_channel=12):
        ...
        if self.concat_feature:
            self.lastconv = nn.Sequential(convbn(320, 128, 3, 1, 1, 1),
                                          nn.ReLU(inplace=True),
                                          nn.Conv2d(128, concat_feature_channel,
...
class hourglass(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv5 = nn.Sequential(
            nn.ConvTranspose3d(in_channels * 4, in_channels * 2, 3, padding=1,
                               output_padding=1, stride=2, bias=False),
            nn.BatchNorm3d(in_channels * 2))

        self.conv6 = nn.Sequential(
            nn.ConvTranspose3d(in_channels * 2, in_channels, 3, padding=1,
                               output_padding=1, stride=2, bias=False),
...
class GwcNet(nn.Module):
    def __init__(self, maxdisp, use_concat_volume=False):
        ...
        self.dres0 = nn.Sequential(convbn_3d(self.num_groups + self.concat_channels * 2,
                                             32, 3, 1, 1),
                                   nn.ReLU(inplace=True),
    ...
    def forward(self, left, right):
        ...
        if self.use_concat_volume:
            concat_volume = build_concat_volume(features_left["concat_feature"],
        ...
        if self.training:
            ...
            cost0 = F.upsample(cost0, [self.maxdisp, left.size()[2],
